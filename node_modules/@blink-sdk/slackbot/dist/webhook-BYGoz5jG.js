...
async function getToken(teamId) {
	const token = await blink.storage.kv.get(getTokenStorageKey(teamId));
	if (!token) return void 0;
...
async function saveToken(oauthResponse) {
	...
	await blink.storage.kv.set(getTokenStorageKey(teamId), JSON.stringify(tokenData));
	return tokenData;
}
...
The user is chatting in Slack. You *must* respond with a "slackbot_send_message" tool.`
		...
		const send = async (behavior) => {
			await blink.chat.message(chatID, {
				...message,
				metadata: {
					...message.metadata ?? {},
					...metadata
		...
		await blink.chat.upsert(chatID);
		await send("enqueue");
