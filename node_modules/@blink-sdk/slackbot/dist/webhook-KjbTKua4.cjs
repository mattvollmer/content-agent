...
async function getToken(teamId) {
	const token = await blink.default.storage.kv.get(getTokenStorageKey(teamId));
	if (!token) return void 0;
...
async function saveToken(oauthResponse) {
	...
	await blink.default.storage.kv.set(getTokenStorageKey(teamId), JSON.stringify(tokenData));
	return tokenData;
}
